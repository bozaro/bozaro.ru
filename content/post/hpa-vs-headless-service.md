...
маршрутизации запросов (ClusterIP: None). В этом случае под DNS именем сервиса видны IP всех Pod-ов, которые в этот
сервис входят.
...
Проблема с остановкой Pod-ов наиболее ярко проявляет себя при перевыкатке сервисов.

Первоначально первопричиной казалось то, что выкатка успевает завершиться быстрее, чем обновляются кэши IP-адресов
сервиса в клиентском приложении (клиент пытается идти на старые Pod-ы которых нет, а про новые он еще не знает).

Для исправления мы просто замедлил выкатку с помощью параметра `minReadySeconds`. Это сделало проблему менее острой, но
не решило её: остались таймауты при подключении к IP для уже не существующего Pod-а.

Тем не менее параметр `minReadySeconds` полезен из-за того, что выкатка не ждёт завершения удаления Pod-а после
перехода его в состояние `Terminating`. В результате при раскатке сервиса мы можем на время добавленных пауз получить x2
Pod-ов.
...
Эта пауза нужна только при корректном завершении Pod-а (в этом случае процесс получает SIGTERM). Если процесс
...
- Между остановкой приложения и завершением Pod-а - чтобы при попытке клиента подключиться/переподключиться к этому
  ...
  адресов только после потери соедния клиента с приложением.
