...
в клиентском приложении (клиент пытается идти на старые Pod-ы которых нет, а про новые он еще не знает).
...
Тем не менее параметр `minReadySeconds` полезен из-за того, что выкатка не ждёт завершения удаления Pod-а после перехода
его в состояние `Terminating`. В результате при раскатке сервиса мы можем на время добавленных пауз получить x2 Pod-ов.
...
подключению к каждому серверу и раскидывает запросы между уже имеющимся подключениями, а не подключаются к сервису при
